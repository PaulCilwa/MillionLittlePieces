...
Partial Public Class Location
    ...
    Protected Sub PerformSearch(Optional StartingPage As Int16 = 1)
        Dim SQL As String = "
        SELECT pd.PagePath, pd.PageDate, pd.DateType
        FROM PageDates pd
        INNER JOIN (
            Select PagePath, MAX(PageDate) AS MaxDate
            FROM PageDates
            GROUP BY PagePath
        ) recentDates ON pd.PagePath = recentDates.PagePath AND pd.PageDate = recentDates.MaxDate
        ORDER BY pd.PageDate DESC"
